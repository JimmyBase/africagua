export default {
  translation: {
    nav: {
      about: 'Cos\'è Africagua?',
      history: 'Storia',
      program: 'Programma',
      b2b: 'Incontri B2B',
      news: 'Notizie',
      useful_info: 'Info utili',
      contact: 'Contatto'
    },
    hero: {
      subtitle: 'Torna Africagua Canarias',
      title1: 'IX Forum dell\'Acqua e delle',
      title2: 'Energie Rinnovabili',
      date: '20-21 novembre 2025',
      register: 'Iscriviti',
      program: 'Programma 2025'
    },
    about: {
      title: 'Cos\'è Africagua?',
      event_section: {
        title: 'Sull\'evento',
        description: 'Africagua è il principale forum internazionale dell\'acqua e delle energie rinnovabili che si celebra a Fuerteventura ogni due anni. Il nostro obiettivo è promuovere l\'incontro tra aziende, esperti e organizzazioni del settore dell\'acqua e delle energie rinnovabili.'
      },
      mission: {
        title: 'La nostra missione',
        description: 'Creiamo sinergie tra Europa e Africa attraverso un networking efficace e incontri B2B strategici, facilitando la collaborazione internazionale nel settore dell\'acqua e delle energie rinnovabili.'
      },
      objectives: [
        {
          title: 'Cooperazione Internazionale in Acqua ed Energia',
          description: 'Promuoviamo sinergie per lo sviluppo sostenibile tra i paesi partecipanti attraverso la collaborazione nel campo dell\'acqua e delle energie rinnovabili.'
        },
        {
          title: 'Hub Imprenditoriale nelle Canarie',
          description: 'Posizioniamo le Canarie e Fuerteventura come centro strategico per l\'interazione tra aziende, partner e istituzioni a livello intercontinentale.'
        },
        {
          title: 'Opportunità di Business Internazionale',
          description: 'Facilitiamo l\'internazionalizzazione attraverso alleanze strategiche e opportunità di business nel settore dell\'acqua e delle energie rinnovabili.'
        },
        {
          title: 'Innovazione e Sviluppo Sostenibile',
          description: 'Promuoviamo il trasferimento di tecnologia, conoscenza e migliori pratiche nella gestione di risorse rinnovabili e sostenibili.'
        }
      ]
    },
    location: {
      title: 'Perché si celebra a Fuerteventura?',
      description: 'Fuerteventura è porta e ponte con l\'Africa per la sua situazione geostrategica. Inoltre, le isole Canarie godono di una situazione di stabilità politica, sicurezza giuridica e un regime fiscale differenziato e approvato dall\'Unione Europea che rendono questo territorio un punto chiave di unione e accordo per questo tipo di iniziative.',
      venue: {
        title: 'Sede dell\'evento',
        name: 'Hotel Sheraton Fuerteventura Beach, Golf & Spa Resort',
        address: 'Av. de las Marismas, 1, 35610 Antigua, Las Palmas',
        viewOnMaps: 'Vedi su Google Maps'
      },
      advantages: [
        {
          title: 'Ubicazione Strategica',
          description: 'Punto di incontro naturale tra Europa e Africa'
        },
        {
          title: 'Risorse Naturali',
          description: 'Leader nelle energie rinnovabili e gestione dell\'acqua'
        },
        {
          title: 'Sicurezza Giuridica',
          description: 'Quadro legale stabile e garantito dall\'UE'
        },
        {
          title: 'Vantaggi Fiscali',
          description: 'Regime fiscale speciale e vantaggioso'
        }
      ]
    },
    b2b: {
      title: 'Incontri B2B ad Africagua 2025',
      description: 'Africagua Canarias 2025 facilita incontri B2B strategici tra aziende del settore dell\'acqua e delle energie rinnovabili. Il nostro obiettivo è promuovere l\'incontro tra aziende, esperti e organizzazioni del settore, creando sinergie tra Europa e Africa attraverso Fuerteventura come punto di incontro internazionale.',
      participation: {
        title: 'Partecipa agli Incontri B2B di Africagua 2025',
        steps: [
          'Completa la tua registrazione sulla piattaforma B2B di Africagua 2025 per accedere agli incontri d\'affari internazionali.',
          'Crea un profilo commerciale dettagliato evidenziando i tuoi obiettivi, prodotti e servizi.',
          'Esplora i profili delle aziende partecipanti e trova potenziali partner per collaborazioni internazionali.',
          'Gestisci le tue richieste di incontro e conferma gli incontri B2B che meglio si adattano ai tuoi obiettivi commerciali.',
          'Partecipa agli incontri di persona durante Africagua 2025 a Fuerteventura, il punto di incontro tra Europa e Africa.'
        ],
        register: 'Registrati per gli incontri B2B'
      }
    },
    b2b_registration: {
      title: 'Registrati per gli Incontri B2B',
      button: 'Registrati per Incontri B2B'
    },
    news: {
      title: 'Notizie',
      no_news: 'Nessuna notizia disponibile al momento.',
      read_more: 'Leggi di più'
    },
    sponsors: {
      title: 'Sponsor',
      benefits: {
        title: 'Benefici della Sponsorizzazione',
        list: [
          'Incorporazione del logo sul sito web e materiali di Africagua',
          'Contenuto esclusivo sui canali di comunicazione',
          'Menzioni in eventi e media',
          'Spazio privilegiato all\'evento',
          'Video promozionale durante le presentazioni',
          'Intervista con i media',
          'Presenza nelle azioni Inter-forum fino al 2025'
        ]
      },
      why_sponsor: [
        {
          title: 'Posizionamento Strategico',
          description: 'Promuovi il tuo marchio in un forum consolidato dell\'acqua e delle energie rinnovabili con portata internazionale.'
        },
        {
          title: 'Networking Globale',
          description: 'Connettiti con potenziali partner e investitori dalle Canarie, Spagna e Africa Occidentale.'
        },
        {
          title: 'Visibilità Premium',
          description: 'Massimizza la tua esposizione mediatica e eleva il profilo del tuo marchio durante e dopo l\'evento.'
        }
      ],
      become_sponsor: 'Diventa Sponsor'
    },
    useful_info: {
      title: 'Informazioni Utili',
      taxis: {
        title: 'Taxi',
        items: [
          {
            name: 'Radio Taxi Puerto del Rosario',
            phone: '928 85 10 10',
            info: 'Servizio 24 ore'
          },
          {
            name: 'Taxi Puerto del Rosario',
            phone: '928 85 33 55',
            info: 'Fermata principale a Plaza de la Paz'
          }
        ]
      },
      buses: {
        title: 'Autobus',
        items: [
          {
            line: 'Linea 06',
            route: 'Puerto del Rosario - Corralejo',
            frequency: 'Ogni 30 minuti'
          },
          {
            line: 'Linea 03',
            route: 'Puerto del Rosario - Caleta de Fuste',
            frequency: 'Ogni ora'
          }
        ],
        schedules: 'Visualizza orari'
      },
      hospital: {
        title: 'Ospedale',
        name: 'Ospedale Generale di Fuerteventura',
        address: 'Carretera del Aeropuerto, Km 1, 35600 Puerto del Rosario',
        phone: '928 862 000',
        emergency: 'Emergenze: 112'
      }
    },
    contact: {
      title: 'Contatto',
      office: {
        title: 'UFFICIO TECNICO',
        address: {
          label: 'Indirizzo',
          line1: 'Camera di Commercio, Industria, Servizi e Navigazione di Fuerteventura',
          line2: 'C/ Secundino Alonso, 98 1° Piano',
          line3: '35600 – Puerto del Rosario, Fuerteventura'
        },
        phone: {
          label: 'Telefono',
          number: '+34 928 861 070'
        },
        email: {
          label: 'Email',
          address: 'info@africagua.com'
        }
      },
      registration: {
        title: 'Iscriviti ad Africagua 2025!',
        description: 'Non perdere l\'opportunità di partecipare al principale forum internazionale dell\'acqua e delle energie rinnovabili. Registrati ora per assicurarti il posto e accedere a tutte le attività, inclusi gli incontri B2B.',
        button: 'Accedi al modulo di registrazione',
        terms: 'Registrandoti, accetti i nostri termini e condizioni di partecipazione.'
      }
    },
    program: {
      title: 'Struttura del programma',
      blocks: {
        sustainability: 'Sostenibilità',
        innovation: 'Innovazione',
        social: 'Sociale',
        governance: 'Governance e finanziamento'
      },
      agenda: {
        title: 'AGENDA 2025',
        soon: 'Prossimamente',
        note: '*Questo programma è soggetto a modifiche'
      }
    },
    speakers: {
      title: 'Relatori'
    },
    program_page: {
      title: 'Programma Africagua 2025',
      subtitle: 'IX Forum Internazionale dell\'Acqua e delle Energie Rinnovabili',
      view_on_map: 'Visualizza su Google Maps',
      days: {
        day1: {
          date: '20 novembre 2025',
          venue: 'Sala Sinfonica',
          sessions: [
            {
              time: '9:00 – 10:00',
              title: 'Cerimonia di apertura',
              type: 'opening',
              participants: [
                'Sua Eccellenza il Sig. Fernando Clavijo Batlle, Presidente del Governo delle Canarie',
                'Sua Eccellenza il Sig. Lamin Camara, Segretario Permanente del Ministero del Petrolio e dell\'Energia del Gambia',
                'Sua Eccellenza la Sig.ra Lola García Martínez, Presidente del Consiglio Insulare di Fuerteventura',
                'Sua Eccellenza il Sig. Cristóbal David de Vera Cabrera, Sindaco di Puerto del Rosario',
                'Presidente della Camera Ufficiale di Commercio, Industria, Servizi e Navigazione di Fuerteventura | Juan Jesús Rodríguez Marichal'
              ]
            },
            {
              time: '10:00 – 11:00',
              title: 'Gestione Integrale dell\'Acqua: Politiche, Sfide e Strategie Regionali',
              type: 'panel',
              moderator: 'Cheik Diouf Faye, Direttore di RAESA in Senegal',
              participants: [
                'Direttore del Ministero delle Risorse Idriche, Lavori e Abitazioni del Ghana | Kwabena Gyasi-Duku',
                'Coordinatore Tecnico della Società Nazionale delle Acque del Senegal (SONES) | El Hadj Ada NDAO',
                'Consigliere del Primo Ministro responsabile dell\'Idraulica e dei Servizi Igienico-Sanitari | Mohamed Kankou',
                'Direttore Esecutivo del Consiglio di Amministrazione dell\'Agenzia Nazionale per l\'Acqua e i Servizi Igienico-Sanitari di Capo Verde (ANAS) | Hipólito Gonçalves',
                'Direttore Generale dell\'Ufficio Nazionale dell\'Elettricità e dell\'Acqua Potabile (ONEE) | Tarik Hammane (in attesa)'
              ]
            },
            {
              time: '11:00 – 11:30',
              title: 'Pausa caffè',
              type: 'break'
            }
          ],
          sala_a: {
            venue: 'Sala A',
            sessions: [
              {
                time: '11:30 – 12:30',
                title: 'Transizione Energetica e Sviluppo delle Infrastrutture energetiche',
                type: 'panel',
                moderator: 'Luis Padilla Macabeo, Direttore Generale delle Relazioni con l\'Africa del Governo delle Canarie',
                participants: [
                  'Segretario Permanente del Ministero del Petrolio e dell\'Energia del Gambia | Lamin Camara',
                  'Direttore dell\'Agenzia Senegalese di Elettricità Rurale (ASER) | Jean Michel Sené',
                  'Vicepresidente della Federazione dell\'Energia e dell\'Elettricità della Mauritania e Direttore Generale del Gruppo SOMACOS S.A | Moktar Mohamed el Moctar',
                  'Responsabile delle Politiche per l\'Energia Sostenibile, Centro ECOWAS per le Energie Rinnovabili e l\'Efficienza Energetica (ECREEE) | Hyacinth Elayo'
                ]
              },
              {
                time: '12:30 – 13:30',
                title: 'Finanziamento di Progetti. Un caso pratico: il dissalatore di Casablanca',
                type: 'panel',
                moderator: 'Loreto Taborga Curto, direttrice territoriale del Commercio Estero e delegata ICEX a Las Palmas',
                participants: [
                  'Sottodirrettrice Generale Aggiunta degli Strumenti Finanziari per l\'Internazionalizzazione di FIEM | María Soledad Sendagorta Fernández-Durán',
                  'Vicedirettrice dei Finanziamenti Strutturati di COFIDES | Ane Sáenz de Zaitegui Bermeo',
                  'Tecnico dell\'Area di Gestione dei Fondi dello Stato per l\'Internazionalizzazione dell\'ICO | Iciar La Casta Muñoa',
                  'Acciona, Direttrice Finanziaria | Eva González Moreno'
                ]
              },
              {
                time: '13:30 – 15:15',
                title: 'Pranzo networking',
                type: 'break'
              },
              {
                time: '15:15 – 16:30',
                title: 'Tavola 4: Alleanze strategiche e cooperazione imprenditoriale e tecnologica',
                type: 'panel',
                moderator: 'María José Miranda Martel, Direttrice del Trasferimento di Conoscenze e Relazioni con le Imprese dell\'Università di Las Palmas de Gran Canaria',
                participants: [
                  'Co-presidente del Comitato Bilaterale Mauritania-Spagna | Ana Suárez Calvo',
                  'Direttore di R&S&I dell\'Istituto Tecnologico delle Canarie (ITC) | Gonzalo Piernavieja Izquierdo',
                  'Coordinatore del Gruppo di Ricerca sui Sistemi Energetici Rinnovabili dell\'ULPGC | Pedro Jesús Cabrera Santana',
                  'Óscar Luengo Barrera, Direttore Generale Welysis'
                ]
              },
              {
                time: '16:30',
                title: 'Chiusura della giornata',
                type: 'break'
              },
              {
                time: '20:30',
                title: 'Cocktail di benvenuto',
                type: 'break',
                description: 'Cocktail di benvenuto per i partecipanti all\'evento. Si terrà presso l\'Hotel Sheraton Fuerteventura Beach, Golf & Spa Resort, Av. de las Marismas, 1, 35610 Antigua, Las Palmas. Nella Sala Tenerife alle 20:45.',
                location: 'Hotel Sheraton Fuerteventura Beach, Golf & Spa Resort',
                locationAddress: 'Av. de las Marismas, 1, 35610 Antigua, Las Palmas',
                mapLink: 'https://www.google.com/maps/place/Sheraton+Fuerteventura+Beach,+Golf+%26+Spa+Resort/@28.3872431,-13.8686424,17z/data=!3m1!4b1!4m9!3m8!1s0xc46bfdb9a5c6c5d:0x8c9e1c9e1c9e1c9e!5m2!4m1!1i2!8m2!3d28.3872431!4d-13.8660675!16s%2Fg%2F1tdxr3l1?entry=ttu'
              }
            ]
          }
        },
        day2: {
          date: '21 novembre 2025',
          venue: 'Sala A',
          sessions: [
            {
              time: '9:00 – 10:00',
              title: 'Gare d\'appalto e Contrattazione Internazionale: Opportunità di business',
              type: 'panel',
              moderator: 'Francisco Daniel Martínez, Addetto Commerciale della Mauritania',
              participants: [
                'Direttore Progetti di GESPLAN | Miguel Ángel Pérez',
                'Responsabile dell\'Antenna ICEX Multilaterali in Costa d\'Avorio | Diego Uriel Gálvez',
                'Tecnico di Promozione Estera di Canarias Aporta PROEXCA | Dolores Hernández Arenciba',
                'Consigliere Economico e Commerciale dell\'Ufficio Economico e Commerciale di Spagna a Dakar | Daniel Jacobo Orea'
              ]
            },
            {
              time: '10:00 – 11:00',
              title: 'Pausa caffè',
              type: 'break'
            },
            {
              time: '11:30 – 12:30',
              title: 'Presentazioni finalisti Global startup competition',
              type: 'presentation',
              description: 'Le presentazioni del Global Startup Competition riuniranno i progetti finalisti di diverse startup internazionali. I team presenteranno le loro iniziative sia in presenza che online, poiché le aziende partecipanti provengono da paesi come Marocco, Etiopia, Kenya, Spagna, India e Congo. Sarà un\'opportunità per scoprire proposte innovative e connettersi con talenti imprenditoriali di tutto il mondo.'
            },
            {
              time: '12:30 – 13:00',
              title: 'Deliberazione giuria',
              type: 'break'
            },
            {
              time: '13:00 – 13:30',
              title: 'Cerimonia di consegna dei Premi Global Startup Competition',
              type: 'awards',
              participants: [
                'Presidente della Camera di Commercio | Juan Jesús Rodríguez Marichal',
                'Javier Franco Hormiga, Agenzia Canaria per la Ricerca, l\'Innovazione e la Società dell\'Informazione'
              ]
            }
          ]
        }
      }
    },
    history: {
      title: 'Storia di Africagua',
      general: {
        title: 'Africagua Canarias – Forum Internazionale dell\'Acqua e delle Energie Rinnovabili',
        first_edition_label: 'Prima edizione:',
        first_edition: '2011',
        frequency_label: 'Frequenza:',
        frequency: 'Biennale',
        location_label: 'Luogo:',
        location: 'Fuerteventura, Isole Canarie',
        organizer_label: 'Organizzatore:',
        organizer: 'Camera di Commercio di Fuerteventura, con il supporto del Governo delle Canarie e del Consiglio Insulare'
      },
      featured_editions: 'Edizioni di Rilievo',
      editions: [
        {
          year: '2023',
          title: 'VIII Edizione - Scambio per il cambiamento',
          description: 'Partecipazione record con delegazioni di 8 paesi africani e focus sui casi di successo aziendale.',
          details: [
            'Date: 23-24 novembre 2023',
            'Partecipanti: Ghana, Mauritania, Gambia, Senegal, Capo Verde, Marocco, Costa d\'Avorio e São Tomé e Príncipe',
            'Formato: Workshop pratici, storie di successo, networking B2B',
            'Casi di rilievo: NOVA STUDIO (vincitore di Africagua Accelerator), Solo H2O, GESPLAN, Cajamar Innova',
            'Partecipanti: 303 persone registrate',
            'Incontri B2B: 84 incontri d\'affari'
          ]
        },
        {
          year: '2021',
          title: 'VII Edizione - Formato Ibrido',
          description: 'Prima edizione con formato ibrido, adattandosi alla nuova realtà post-pandemia.',
          details: [
            'Date: 30 novembre - 1 dicembre 2021',
            'Formato: In presenza + streaming online',
            'Temi chiave: Transizione energetica, digitalizzazione dell\'acqua, cooperazione internazionale',
            'Partecipazione: Delegazioni dell\'Africa Occidentale',
            'Istituzioni: Rappresentanti europei e aziende tecnologiche'
          ]
        },
        {
          year: '2019',
          title: 'VI Edizione - Economia Blu',
          description: 'Focus sull\'economia blu e la cooperazione triangolare per lo sviluppo sostenibile.',
          details: [
            'Focus principale: Economia blu e sostenibilità marina',
            'Cooperazione triangolare: Europa-Africa-Canarie',
            'Paesi partecipanti: Senegal, Capo Verde, Marocco, Mauritania',
            'Tematiche: Gestione sostenibile delle risorse marine',
            'Innovazione: Nuove tecnologie per l\'economia blu'
          ]
        },
        {
          year: '2017',
          title: 'V Edizione - Consolidamento Internazionale',
          description: 'Edizione consolidata come importante forum per la cooperazione internazionale nelle energie rinnovabili.',
          details: [
            'Temi centrali: Acqua, energie pulite, opportunità di investimento in Africa',
            'Partecipazione: Più di 200 partecipanti',
            'Rappresentanti: ECOWAS e Unione Europea',
            'Progetti: Presentazione di iniziative di investimento',
            'Networking: Intensificazione dei contatti commerciali'
          ]
        }
      ],
      impact: {
        title: 'Impatto e Obiettivi',
        description: 'Africagua si è affermato come ponte strategico tra Africa ed Europa, con le Canarie come hub chiave.',
        objectives: [
          'Favorire il trasferimento di conoscenze tra continenti',
          'Spingere l\'internazionalizzazione delle aziende canarie',
          'Promuovere la cooperazione Sud-Sud e Nord-Sud',
          'Sostenere l\'innovazione nell\'acqua e nelle energie rinnovabili'
        ]
      }
    },
    speaker_profiles: {
      'Kwabena Gyasi-Duku': {
        name: 'Kwabena Gyasi-Duku',
        title: 'Direttore del Ministero delle Risorse Idriche, Lavori e Abitazioni del Ghana',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FKwabena%20Asare%20Gyasi.png?alt=media&token=af180bbf-9eb8-411b-b4a3-0384e9425168',
        bio: 'Ingegnere civile con esperienza nell\'approvvigionamento idrico e nell\'ingegneria ambientale, con lauree presso l\'Università di Scienza e Tecnologia Kwame Nkrumah in Ghana e la Technische Universität Bergakademie Freiberg in Germania. Attualmente dirige la Direzione delle Acque del Ministero dei Lavori, dell\'Edilizia e delle Risorse Idriche del Ghana, portando oltre 20 anni di esperienza nella progettazione e implementazione di progetti di approvvigionamento idrico, servizi igienico-sanitari e ambiente nei settori pubblico e privato. Rappresenta il Ghana in numerose piattaforme locali e internazionali, incluso il ruolo di punto focale del paese per l\'Obiettivo di Sviluppo Sostenibile 6 e come membro del Comitato Consultivo Tecnico del Consiglio dei Ministri Africani sull\'Acqua (AMCOW). È anche co-responsabile dell\'impegno del Ghana nell\'iniziativa Servizi Igienico-Sanitari e Acqua per Tutti (SWA).'
      },
      'Mohamed Kankou': {
        name: 'Mohamed Kankou',
        title: 'Consigliere del Primo Ministro responsabile dell\'Idraulica e dei Servizi Igienico-Sanitari',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FMohamed%20Kankou.png?alt=media&token=8a962404-72b3-46f0-9678-210c372f7f46',
        bio: 'Consigliere del Primo Ministro responsabile dell\'Idraulica e dei Servizi Igienico-Sanitari dall\'08/06/2022 ad oggi. Specializzato in Chimica dell\'acqua, Chimica organica e sintesi, Cinetica chimica, Chimica analitica, Tecnologia delle sostanze naturali e Chimica industriale. Coordinatore dell\'Unità di Ricerca: Acqua, Inquinamento e Ambiente: Direzione di diverse Tesi di Dottorato e Tesi di Master nell\'ambito dell\'Acqua e dell\'Ambiente.'
      },
      'Hipólito Gonçalves': {
        name: 'Hipólito Gonçalves',
        title: 'Direttore Esecutivo del Consiglio di Amministrazione dell\'Agenzia Nazionale per l\'Acqua e i Servizi Igienico-Sanitari di Capo Verde (ANAS)',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FHip%C3%B3lito%20Gon%C3%A7alves.png?alt=media&token=a781e4b1-7ce0-489d-8f22-2437e3c4ffcf',
        bio: 'Ingegnere meccanico specializzato in Energia e Termodinamica, laureato presso l\'Istituto Superiore di Ingegneria di Lisbona. Dal febbraio 2025 ricopre la carica di Amministratore Esecutivo dell\'Agenzia Nazionale per l\'Acqua e i Servizi Igienico-Sanitari (ANAS) di Capo Verde, dove guida le aree tecniche e strategiche del settore idrico e igienico-sanitario. Con oltre due decenni di esperienza in ingegneria, gestione di progetti e insegnamento universitario, ha lavorato a iniziative chiave come il Progetto di Sviluppo dei Sistemi di Approvvigionamento Idrico sull\'Isola di Santiago, finanziato dalla JICA. È anche professore presso l\'Università di Capo Verde e ha partecipato a varie formazioni internazionali in dissalazione, energie rinnovabili e gestione delle risorse idriche. Il suo lavoro si concentra sulla promozione della sostenibilità, dell\'innovazione e della modernizzazione della gestione pubblica dell\'acqua nel paese.'
      },
      'Hyacinth Elayo': {
        name: 'Hyacinth Elayo',
        title: 'Responsabile delle Politiche per l\'Energia Sostenibile, Centro ECOWAS per le Energie Rinnovabili e l\'Efficienza Energetica (ECREEE)',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FHyacinth%20Elayo.png?alt=media&token=e44136b3-89c5-4679-b6dc-e18409bcb366',
        bio: 'Hyacinth Elayo è Responsabile delle Politiche per l\'Energia Sostenibile presso il Centro ECOWAS per le Energie Rinnovabili e l\'Efficienza Energetica (ECREEE), con sede a Praia, Capo Verde. Guida il Programma di Politiche per l\'Energia Sostenibile dell\'ECOWAS, promuovendo iniziative che favoriscono investimenti in energia sostenibile negli stati membri dell\'ECOWAS. Con oltre 15 anni di esperienza nel settore energetico, possiede ampie competenze in ricerca, formulazione di politiche energetiche e promozione di soluzioni energetiche accessibili per promuovere lo sviluppo socio-economico. Possiede un Master in Relazioni Internazionali dall\'Università di Lancaster e un Master in Studi Energetici dall\'Università di Dundee.'
      },
      'María Soledad Sendagorta Fernández-Durán': {
        name: 'María Soledad Sendagorta Fernández-Durán',
        title: 'Sottodirrettrice Generale Aggiunta degli Strumenti Finanziari per l\'Internazionalizzazione di FIEM',
        image: '',
        bio: 'Sottodirrettrice Generale Aggiunta nella Sottodirezione Generale degli Strumenti Finanziari per l\'Internazionalizzazione. Responsabile della strutturazione e gestione di progetti con partecipazione del FIEM secondo schemi di project finance, il suo lavoro comprende anche l\'intervento in negoziazioni internazionali sia a livello europeo che nell\'ambito dell\'OCSE. Questa è l\'unità responsabile del coordinamento del sistema di strumenti finanziari che la Segreteria di Stato per il Commercio mette a disposizione delle imprese spagnole per sostenerle nei loro progetti di esportazione e investimento. Questi strumenti includono il finanziamento diretto fornito dal Fondo per l\'Internazionalizzazione dell\'Impresa (FIEM) così come i fondi FIEX e FONPYME gestiti da COFIDES e la copertura dei rischi per conto dello Stato gestita da Cesce.'
      },
      'Ane Sáenz de Zaitegui Bermeo': {
        name: 'Ane Sáenz de Zaitegui Bermeo',
        title: 'Vicedirettrice dei Finanziamenti Strutturati di COFIDES',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FANE%20SAENZ.png?alt=media&token=4f598de2-3d87-4855-9568-78e8a207a39e',
        bio: 'Professionista del settore finanziario con una solida esperienza in analisi economico-finanziaria, strutturazione e finanziamento di progetti, in particolare nell\'ambito del Project Finance e dei Partenariati Pubblico-Privati (PPP). Attualmente fa parte del team di COFIDES, dove ha ampliato le sue competenze in analisi del credito e finanziamento di progetti a livello internazionale, partecipando attivamente a operazioni di debito e capitale nei settori delle infrastrutture e dell\'energia in vari mercati.'
      },
      'Iciar La Casta Muñoa': {
        name: 'Iciar La Casta Muñoa',
        title: 'Tecnico dell\'Area di Gestione dei Fondi dello Stato per l\'Internazionalizzazione dell\'Istituto di Credito Ufficiale di Spagna',
        image: '',
        bio: 'Laureata in Scienze Economiche e Aziendali presso CUNEF, è Tecnico nell\'Area di Gestione dei Fondi dello Stato per l\'Internazionalizzazione dell\'Istituto di Credito Ufficiale di Spagna, nel suo ruolo di Agenzia Finanziaria dello Stato, area in cui lavora nel Front Office delle operazioni dei Fondi dello Stato (FIEM e FEDES) da dieci anni. In precedenza, dal 2009 al 2015, è stata Responsabile del Dipartimento di Politiche e Ammissione dei Rischi di Credito dell\'ICO, nel suo ruolo di Banca Nazionale di Promozione, e Membro del Comitato di Investimento di FOND ICO PYME de AXIS, del Gruppo ICO, dopo nove anni in Analisi dei Rischi di Credito, sia Corporate che Project Finance, presso l\'Istituto e, prima di entrare in ICO, presso l\'istituzione finanziaria BNP Paribas. In BNP Paribas, è stata Responsabile della Divisione Controllo Rischi e Reporting e, in precedenza, Responsabile del Team di Audit presso KPMG.'
      },
      'Ana Suárez Calvo': {
        name: 'Ana B. Suárez Calvo',
        title: 'Copresidente del Comitato Bilaterale Spagna-Mauritania e Presidente del Consiglio Sociale dell\'ULPGC',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FANA%20SUAREZ%20CALVO.png?alt=media&token=c3a2ad83-b315-4f87-91a9-000216df5414',
        bio: 'Laureata in Scienze Economiche e Aziendali presso l\'Università Pontificia di Comillas, dal 2024 è la Presidente del Consiglio Sociale dell\'ULPGC e Copresidente, dalla parte spagnola, del Comitato Bilaterale Spagna-Mauritania, in qualità di Membro della Camera di Spagna. Guida il gruppo SUARIS Capital, family office con presenza in settori strategici delle Canarie come il turismo, il trasporto aereo, le nuove tecnologie, il settore sanitario e la produzione audiovisiva.'
      },
      'Pedro Jesús Cabrera Santana': {
        name: 'Pedro Jesús Cabrera Santana',
        title: 'Coordinatore del Gruppo di Ricerca sui Sistemi Energetici Rinnovabili dell\'Università di Las Palmas de Gran Canaria',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FPedro%20Jes%C3%BAs%20Cabrera.png?alt=media&token=437bc603-08cf-44f5-a810-5409a679b7df',
        bio: 'Professore Ordinario del Dipartimento di Ingegneria Meccanica dell\'ULPGC dell\'Università di Las Palmas de Gran Canaria (ULPGC), dove coordina il Group for the Research on Renewable Energy Systems (GRRES). Ha ricoperto cariche di responsabilità come Coordinatore Nazionale del Gruppo Tematico di Controllo Intelligente del Comitato Spagnolo di Automatica (2020-2024), Editore della Sezione di Controllo Intelligente della Rivista Ibero-americana di Automatica e Informatica Industriale (RIAI) e Segretario Tecnico della Piattaforma R&S&I DESAL+ LIVING LAB, riferimento internazionale nell\'innovazione nella dissalazione. Autore e co-autore di multiple pubblicazioni scientifiche e libri, il suo lavoro combina ricerca applicata, trasferimento tecnologico e sostegno all\'imprenditorialità nel settore delle energie pulite. Attualmente guida lo sviluppo di strumenti e metodologie che contribuiscono alla pianificazione di sistemi energetici sostenibili a scala nazionale e internazionale.'
      },
      'Jorge Andrés Rodríguez Navarro': {
        name: 'Jorge Andrés Rodríguez Navarro',
        title: 'Professore di Chimica Inorganica presso l\'Università di Granada',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FJorge%20Andr%C3%A9s%20Rodr%C3%ADguez%20Navarro.png?alt=media&token=6d106076-5ea2-4bd4-b202-815bfc4e60b2',
        bio: 'Professore di Chimica Inorganica presso l\'Università di Granada. Ha realizzato contributi pionieristici nel campo della sintesi di architetture metallo-organiche discrete ed estese con applicazioni biomediche avanzate (rilascio controllato di farmaci e gas segnale) e nella bonifica ambientale e cambiamento climatico (cattura di CO2 e SO2 e degradazione di composti tossici). Questi risultati sono stati pubblicati in riviste internazionali ad alta diffusione tra cui Nature, Science, Journal of the American Chemical Society, Advanced Materials, ricevendo quasi 11.000 citazioni. È stato professore invitato presso le Università di Wroclaw (2025), UC Berkeley (2022, 2013), U. Versailles (2016) ed è cofondatore della Spin-off Porous Materials in Action.'
      },
      'Diego Uriel Gálvez': {
        name: 'Diego Uriel Gálvez',
        title: 'Responsabile dell\'Antenna Multilaterale ICEX',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FDiego%20Uriel%20G%C3%A1lvez.png?alt=media&token=4176033e-a987-4835-ab05-570a57f634e4',
        bio: 'Laureato in Economia presso l\'Università Carlos III di Madrid. Blue Book alla Commissione Europea in applicazioni di mercato per il sistema Galileo ed EGNOS. Successivamente Analista di Mercato presso l\'Ufficio Economico e Commerciale di Spagna a Pechino per sette anni, prima nel Dipartimento di agribusiness e poi per Invest in Spain. Dal 2023, professore nel master di Geopolitica e Analisi Internazionale presso LISA Institute e volontario sul campo in progetti di cooperazione internazionale. Attualmente ricopre la posizione di Capo Dipartimento delle Istituzioni Finanziarie Multilaterali per ICEX presso l\'Ufficio Economico e Commerciale di Spagna ad Abidjan.'
      },
      'Mar Naranjo Suárez': {
        name: 'Mar Naranjo Suárez',
        title: 'Area di Promozione Estera',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FMar%20Naranjo%20Suarez.png?alt=media&token=6a54a61c-3ae3-4346-af0e-de9194e06f4d',
        bio: 'Con formazione in Giurisprudenza, Diplomazia e Relazioni Internazionali, Internazionalizzazione di Imprese e Amministrazione e Direzione di Imprese, ha iniziato la sua carriera professionale come rappresentante legale della Fondazione Canaria per l\'Azione Estera (Fucaex) in Mauritania, dove ha lavorato nella promozione economica e nella cooperazione istituzionale e imprenditoriale tra le Canarie e questo paese dell\'Africa Occidentale. Con oltre sette anni di esperienza nello sviluppo di business internazionale, ha guidato progetti di espansione, attrazione di investimenti e promozione estera in settori strategici della regione. Attualmente, come parte del team dell\'Area di Promozione Estera di Proexca, concentra il suo lavoro sulla promozione delle imprese canarie all\'estero, promuovendo, tra gli altri settori, l\'esportazione della conoscenza e dell\'esperienza del nostro tessuto imprenditoriale.'
      },
      'Silvia Camacho Martín': {
        name: 'Silvia Camacho Martín',
        title: 'Tecnico in Promozione Estera presso Proexca',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FSEGUNDA%20TANDA%2FSilvia%20Camacho%20-%20Mart%C3%ADn.png?alt=media&token=506943ff-cf22-424a-a3df-680c27cfcab7',
        bio: 'Tecnico in Promozione Estera presso Proexca. È stata coordinatrice del programma di gare d\'appalto internazionali per 3 anni. Ha visitato diverse organizzazioni multilaterali, come la sede della Banca Mondiale a Washington D.C. e la Banca Africana di Sviluppo ad Abidjan, familiarizzando con le loro politiche, normative e priorità, nonché con i paesi beneficiari in cui sono previsti o in fase di sviluppo progetti. Ha fatto parte di reti globali per l\'individuazione di opportunità e l\'avvicinamento tra partner nella partecipazione ai processi di gara internazionali. In precedenza, ha ricoperto altri ruoli chiave in questo ambito, come responsabile dello sviluppo del business internazionale. Silvia possiede un Master in International Business e una Laurea in Amministrazione e Direzione di Imprese Internazionali, e diverse esperienze professionali a livello internazionale, sempre nell\'ambito dello sviluppo del business per i settori prioritari dell\'economia canaria.'
      },
      'Daniel Jacobo Orea': {
        name: 'Daniel Jacobo Orea',
        title: 'Consigliere Economico e Commerciale dell\'Ambasciata di Dakar in Spagna',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FDANIEL%20JACOBO%20OREA.png?alt=media&token=c0141f5a-a255-4b56-ab15-7d491d1da744',
        bio: 'È entrato nel 2016 nel corpo dei Tecnici Commerciali ed Economisti dello Stato. Nel 2021, è entrato nella Segreteria di Stato per la Cooperazione Internazionale del MAEC, lavorando come Direttore della Divisione di Valutazione delle Politiche per lo Sviluppo Sostenibile tra il 2022 e il 2024. Da allora, svolge il ruolo di Consigliere Economico e Commerciale dell\'Ambasciata di Spagna a Dakar, con giurisdizione in Senegal, Mauritania, Capo Verde, Gambia, Mali e Guinea-Bissau.'
      },
      'Eva González Moreno': {
        name: 'Eva González Moreno',
        title: 'Direttrice Finanziaria di Acciona',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FSEGUNDA%20TANDA%2FEva%20Gonz%C3%A1lez%20Moreno.png?alt=media&token=2e14761c-cf5e-42c4-be53-b7375ca9a799',
        bio: 'Ha più di 12 anni di esperienza nel settore energetico internazionale, specializzata in strutturazione finanziaria, finanziamento di progetti e direzione aziendale. Ha sviluppato la sua carriera in Colombia, Francia, Spagna e attualmente in Marocco, ricoprendo posizioni chiave in dipartimenti finanziari di aziende leader del settore. Attualmente dirige la Direzione Finanziaria di Al Baidaa Desalination Company, società responsabile del progetto di dissalazione di Casablanca, uno dei più rilevanti in Africa nella sua categoria.'
      },
      'Nani Juwara': {
        name: 'Nani Juwara',
        title: 'Ministro del Petrolio e dell\'Energia, Governo della Repubblica del Gambia',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FSEGUNDA%20TANDA%2FNani%20Juwara.png?alt=media&token=d4f8f07b-2dde-4458-9216-368fb136b8b0',
        bio: 'È un leader con vasta esperienza nel settore energetico del Gambia, supportato da formazione internazionale in amministrazione, sviluppo e gestione strategica. Ha ricoperto posizioni chiave in NAWEC, da Vice Direttore a Direttore Generale, e attualmente ricopre il ruolo di Ministro del Petrolio e dell\'Energia. Nel corso della sua carriera ha guidato importanti progetti di modernizzazione elettrica finanziati da organizzazioni internazionali —tra cui la Banca Mondiale, ECOWAS e AfDB— e ha promosso l\'obiettivo nazionale di raggiungere l\'accesso universale all\'elettricità entro il 2025. La sua traiettoria combina gestione esecutiva, supervisione di progetti infrastrutturali e certificazioni tecniche e finanziarie che rafforzano la sua leadership nello sviluppo energetico del paese.'
      },
      'Lamin Camara': {
        name: 'Lamin Camara',
        title: 'Segretario Permanente del Ministero del Petrolio e dell\'Energia del Gambia',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FSEGUNDA%20TANDA%2FLamin%20Camara.png?alt=media&token=5a2bb33b-9e5c-4e76-b000-dbcd2f8a8e45',
        bio: 'Lamin Camara è un amministratore pubblico di alto livello, riconosciuto per la sua leadership strategica e la sua capacità di trasformare le istituzioni attraverso l\'innovazione tecnologica. Dal 2017 ricopre il ruolo di Segretario Permanente del Ministero del Petrolio e dell\'Energia della Repubblica del Gambia, dove dirige funzioni esecutive, promuove la modernizzazione amministrativa e garantisce l\'esecuzione efficiente delle iniziative governative chiave. Con una solida formazione internazionale in gestione delle comunicazioni e amministrazione aziendale, si distingue per la sua padronanza nella Gestione del Cambiamento e nell\'integrazione di tecnologie che ottimizzano i servizi pubblici. Il suo profilo combina visione strategica, capacità operativa e un fermo impegno per l\'eccellenza nella governance pubblica.'
      },
      'El Hadj Ada NDAO': {
        name: 'El Hadj Ada NDAO',
        title: 'Coordinatore Tecnico della Società Nazionale delle Acque del Senegal (SONES)',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FSEGUNDA%20TANDA%2FEl%20Hadi%20Ada%20Ndao.png?alt=media&token=85d3ffe1-8b77-421c-bb78-de3612dd9b95',
        bio: 'È un ingegnere progettista in ingegneria civile, laureato presso la Scuola Politecnica di Thiès (Senegal). Ha 30 anni di esperienza nella gestione delle risorse idriche, inclusi 10 anni di esperienza come direttore presso la Società Nazionale delle Acque del Senegal, durante i quali ha diretto progetti di sviluppo con banche bilaterali e multilaterali (BM, AFD, BEI, BID, BAD, JICA, BOAD) nelle fasi di identificazione, studi di fattibilità tecnico-economica, valutazioni ambientali e sociali, studi dettagliati, ricerca di finanziamenti, negoziazione di accordi di finanziamento, nonché nella gestione dei lavori e/o nella direzione dei progetti.'
      },
      'Tarik Hamane': {
        name: 'Tarik Hamane',
        title: 'Direttore Generale dell\'Ufficio Nazionale dell\'Elettricità e dell\'Acqua Potabile del Marocco (ONEE)',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FSEGUNDA%20TANDA%2FTARIK%20HAMANE.png?alt=media&token=bb65c530-f183-408a-ab18-967f95d44c4e',
        bio: 'Tarik Hamane è un esperto riconosciuto del settore energetico e attuale Direttore Generale dell\'ONEE, dopo essere stato CEO ad interim di Masen. Ha ricoperto posizioni di alto livello in aziende globali come TOTAL Energies, dove ha diretto lo sviluppo mondiale dell\'idrogeno verde, e TOTAL Eren per il Nord Africa. Ha svolto funzioni chiave presso Masen e l\'ONEE, oltre ad aver iniziato la sua carriera nella ricerca presso il CNRS e il CERN. Parallelamente, ricopre importanti responsabilità internazionali: è Governatore del World Water Council, Vicepresidente di AfWASA e Co-Presidente dell\'OMEC. Presiede anche AMEPA e ha guidato entità come l\'Associazione dei Servizi Pubblici di Elettricità dell\'Africa e l\'Unione Araba dell\'Elettricità. È ingegnere elettrico e automatico presso l\'INSA Lione, con un master in Sistemi di Informazione Industriale.'
      },
      'Moktar Mohamed el Moctar': {
        name: 'Moktar Mohamed el Moctar',
        title: 'Vicepresidente della Federazione di Energia ed Elettricità della Mauritania e Direttore Generale del Gruppo SOMACOS S.A',
        image: '',
        bio: 'Vicepresidente della Federazione di Energia ed Elettricità della Mauritania e Direttore Generale del Gruppo SOMACOS S.A. Leader imprenditoriale con ampia esperienza nel settore energetico mauritano, specializzato nella gestione e nello sviluppo di infrastrutture elettriche. La sua carriera è caratterizzata dalla promozione di progetti di modernizzazione energetica e dalla promozione della cooperazione regionale nel settore dell\'energia e dell\'elettricità.'
      },
      'Jean Michel Sené': {
        name: 'Jean Michel Sené',
        title: 'Direttore dell\'Agenzia Senegalese per l\'Elettrificazione Rurale (ASER)',
        image: '',
        bio: 'Direttore dell\'Agenzia Senegalese per l\'Elettrificazione Rurale (ASER), un\'istituzione chiave nell\'elettrificazione delle aree rurali in Senegal. Con una solida formazione tecnica ed esperienza nel settore energetico, guida progetti di accesso all\'elettricità nelle comunità rurali, promuovendo lo sviluppo sostenibile e l\'integrazione delle energie rinnovabili in aree tradizionalmente poco servite.'
      },
      'Elayo Hyacinth': {
        name: 'Elayo Hyacinth',
        title: 'Responsabile delle Politiche e delle Normative del Centro ECOWAS per le Energie Rinnovabili e l\'Efficienza Energetica (ECREEE)',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FHyacinth%20Elayo.png?alt=media&token=e44136b3-89c5-4679-b6dc-e18409bcb366',
        bio: 'Responsabile delle Politiche Energetiche Sostenibili presso il Centro ECOWAS per le Energie Rinnovabili e l\'Efficienza Energetica (ECREEE), con sede a Praia, Capo Verde. Guida il Programma di Politiche Energetiche Sostenibili dell\'ECOWAS, promuovendo iniziative che favoriscono gli investimenti in energia sostenibile negli stati membri dell\'ECOWAS. Con oltre 15 anni di esperienza nel settore energetico, possiede un\'ampia competenza nella ricerca, formulazione di politiche energetiche e promozione di soluzioni energetiche accessibili per promuovere lo sviluppo socio-economico. Possiede un Master in Relazioni Internazionali presso l\'Università di Lancaster e un Master in Studi Energetici presso l\'Università di Dundee.'
      },
      'Juan Jesús Rodríguez Marichal': {
        name: 'Juan Jesús Rodríguez Marichal',
        title: 'Presidente della Camera di Commercio di Fuerteventura',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FSEGUNDA%20TANDA%2FJUAN%20JESUS.png?alt=media&token=59447b83-82ca-4487-b948-a673b82437f7',
        bio: 'Presidente della Camera di Commercio di Fuerteventura dal 2022. Economista ed esperto di urbanistica, con ampia esperienza nel settore imprenditoriale e delle costruzioni. Ha guidato iniziative per promuovere l\'internazionalizzazione, la connettività e lo sviluppo sostenibile nelle Isole Canarie. Attualmente rappresenta la Camera presso l\'Autorità Portuale e presiede il Consiglio Generale delle Camere di Commercio delle Canarie.'
      },
      'Juan Manuel Revuelta': {
        name: 'Juan Manuel Revuelta',
        title: 'Direttore Generale della Fondazione Finnova',
        image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/PONENTES%20NUEVOS%20PARA%20LA%20WEB%2FSEGUNDA%20TANDA%2FJuan%20Manuel%20Revuelta.png?alt=media&token=cf0d8c58-ca2e-421c-a717-52f30ee97e93',
        bio: 'Con oltre 25 anni di esperienza a Bruxelles, guida progetti europei di cooperazione tra Europa, Isole Canarie e Africa, come iniziative di mobilità sostenibile, energie pulite e trasferimento tecnologico. Sotto la sua direzione, Finnova promuove programmi come StartUp Europe Awards e FU-Tourism Accelerator, oltre a partecipare a progetti LIFE e Horizon Europe che rafforzano la collaborazione internazionale nell\'ambito atlantico e africano.'
      }
    },
    startup_competition: {
      title: 'Concorso Globale di Startup',
      subtitle: 'Promuovere l\'innovazione nell\'acqua e nelle energie rinnovabili',
      description: 'Il Concorso Globale di Startup Africagua cerca startup con progetti innovativi e dirompenti nel campo dell\'acqua e delle energie rinnovabili.',
      registration: {
        open: 'Registrazione chiusa',
        deadline: 'Scadenza: 1 settembre 2025 (Chiuso)',
        download_rules: 'Scarica il regolamento',
        submit_project: 'Invia il tuo progetto'
      },
      prizes: {
        title: 'Premi',
        first: 'Primo premio: 10.000€',
        second: 'Secondo premio: 5.000€'
      },
      training: {
        title: 'Formazione',
        description: 'I 10 progetti finalisti seguiranno una formazione su incentivi fiscali e trasferimento di conoscenze università-impresa'
      },
      finalists: {
        title: 'FINALISTI',
        description: 'Le 10 aziende finaliste presenteranno i loro progetti nel quadro del Forum Africagua Canarias 2025, con 5 minuti per fare il pitch. L\'evento si terrà il 21 novembre 2025, presso il Palazzo di Formazione e Congressi di Fuerteventura.',
        coming_soon: 'PROGRAMMA AFRICAGUA CANARIAS 2025 IN ARRIVO'
      },
      jury: {
        title: 'Giuria del Concorso',
        expert_panel: 'Panel di Esperti',
        description: 'Professionisti riconosciuti valuteranno i progetti partecipanti',
        expert_in: 'Esperto in',
        view_full_bio: 'Visualizza biografia completa',
        members: [
          {
            name: 'Gema Báez Espino',
            position: 'Responsabile Imprese DayOne Canarias Caixabank',
            expertise: 'Consulenza Finanziaria',
            image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/GLOBAL%20STARTUP%2FMIEMBROS%20DE%20JURADO%2FGema%20B%C3%A1ez%20Espino%2C%20Responsable%20Empresas%20DayOne%20Canarias%20Caixabank.jpg?alt=media&token=585785be-081b-4d30-9844-4746a1cd3a1f',
            bio: 'Laureata in Amministrazione e Direzione d\'Impresa (specialità in Direzione Generale e Risorse Umane) dell\'Università di Las Palmas de G.C. ed Esperta in Consulenza Finanziaria, certificata da IEAF e titolo universitario di Specialista in Banca e Consulenza Finanziaria tramite CUNEF.',
            fullBio: 'Laureata in Amministrazione e Direzione d\'Impresa (specialità in Direzione Generale e Risorse Umane) dell\'Università di Las Palmas de G.C. ed Esperta in Consulenza Finanziaria, certificata da IEAF e titolo universitario di Specialista in Banca e Consulenza Finanziaria tramite CUNEF. Gema Báez ha iniziato la sua carriera professionale nel 2005, nel dipartimento Rischi della Caja Rural de Canarias, e nel 2007 inizia la sua carriera professionale in Caja Madrid, oggi Caixabank, passando attraverso diversi posti e promozioni. Oggi è responsabile del segmento DayOne nelle Canarie in Caixabank. DayOne è il servizio bancario specializzato di CaixaBank creato per supportare gli agenti dell\'ecosistema dell\'innovazione. Ha la vocazione di sviluppare e accompagnare tutte le imprese tecnologiche, a sviluppo rapido e ambito di azione globale che realizzano attività a valore aggiunto e che finanziano la crescita nelle loro prime fasi principalmente con fondi propri. In definitiva DayOne è il servizio che innova il modo di relazionarsi con tutti gli agenti che formano l\'ecosistema delle start-up, scale-up e i loro investitori.'
          },
          {
            name: 'Patricia Fraile Romero',
            position: 'Fund Manager di Archipiélago Next',
            expertise: 'Capital Venture',
            image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/GLOBAL%20STARTUP%2FMIEMBROS%20DE%20JURADO%2FPatricia%20Fraile%20Romero%2C%20Fund%20Manager%20de%20Archipi%C3%A9lago%20Next.jpg?alt=media&token=d974dd8f-61ba-4c85-9ec0-63b95d0a087c',
            bio: 'Patricia Fraile è Fund Manager di Archipélago Next, il primo fondo di capital venture con DNA 100% canario. Laureata in Diritto Economico dell\'Università di Deusto, con un Master in Diritto Internazionale e Comparato.',
            fullBio: 'Patricia Fraile è Fund Manager di Archipélago Next, il primo fondo di capital venture con DNA 100% canario. Laureata in Diritto Economico dell\'Università di Deusto, con un Master in Diritto Internazionale e Comparato, Patricia conta su una solida traiettoria nella gestione aziendale e finanziaria. Prima della sua incorporazione ad Archipélago Next, Patricia ha accumulato una vasta esperienza nel settore tecnologico e dell\'innovazione. Tra il 2011 e il 2019, ha svolto il ruolo di manager del Cluster Canarias Excelencia Tecnológica, un raggruppamento che riunisce le imprese del settore TIC nella regione. Durante la sua gestione, il cluster si è consolidato come un riferimento nel settore, spingendo progetti di R&S&I e favorendo la collaborazione tra imprese per promuovere l\'imprenditorialità tecnologica nelle isole. Lavoro che ha anche portato al livello nazionale, con la sua partecipazione attiva in FENAEIC e CONETIC.'
          },
          {
            name: 'Pablo Martín Carbajal',
            position: 'Amministratore Delegato di PROEXCA',
            expertise: 'Relazioni Internazionali',
            image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/GLOBAL%20STARTUP%2FMIEMBROS%20DE%20JURADO%2FPablo%20Martin%20Carbajal%20consejero%20delegado.jfif?alt=media&token=f1c90bf0-a2fd-4372-8edc-29f9a3af54b7',
            bio: 'Pablo Martín Carbajal ha lavorato nell\'ambito delle relazioni internazionali per più di 25 anni. I suoi campi di lavoro sono stati il commercio estero, l\'attrazione di investimenti, le relazioni istituzionali per l\'internazionalizzazione e la cooperazione internazionale.',
            fullBio: 'Pablo Martín Carbajal ha lavorato nell\'ambito delle relazioni internazionali per più di 25 anni. I suoi campi di lavoro sono stati il commercio estero, l\'attrazione di investimenti, le relazioni istituzionali per l\'internazionalizzazione e la cooperazione internazionale. Con vasta esperienza nei mercati africani vicini alle Canarie è stato per 12 anni direttore generale delle relazioni con l\'Africa del Governo delle Canarie. Attualmente sta compiendo il suo secondo mandato come Amministratore Delegato di Proexca, l\'impresa pubblica per l\'internazionalizzazione del Governo canario, dove accumula un\'esperienza di 10 anni lavorando per l\'internazionalizzazione dell\'economia dell\'arcipelago. Pablo parla inglese e francese e ha lavorato e vissuto in Francia, Belgio, Armenia e Georgia.'
          },
          {
            name: 'Eduardo Pereira González',
            position: 'Manager del Parco Tecnologico di Fuerteventura',
            expertise: 'Innovazione Tecnologica',
            image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/GLOBAL%20STARTUP%2FMIEMBROS%20DE%20JURADO%2FEduardo%20Pereira%20Gonz%C3%A1lez%2C%20gerente%20del%20Parque%20Tecnol%C3%B3gico%20de%20Fuerteventura.jpg?alt=media&token=09e4e7a3-e576-4ea4-b72c-068a60329f99',
            bio: 'Eduardo Pereira, nato a Carballiño, provincia di Ourense nel 1963, è ingegnere aerospaziale dell\'Università Politecnica di Madrid e membro in congedo del Corpo di Ingegneri Aeronautici dello Stato.',
            fullBio: 'Eduardo Pereira, nato a Carballiño, provincia di Ourense nel 1963, è ingegnere aerospaziale dell\'Università Politecnica di Madrid e membro in congedo del Corpo di Ingegneri Aeronautici dello Stato. Possiede un Master in Business Administration e un Master in Contratti Pubblici, oltre ad essere diplomato in Direzione Finanziaria e Controllo di Gestione. Ha occupato cariche di grande rilevanza, come direttore degli aeroporti di Santiago de Compostela nel 1994 e di Fuerteventura tra il 1994 e il 2006, dove ha guidato il progetto di espansione dell\'aeroporto con un investimento accumulato di più di 500 milioni di euro. Dal 2010, è manager del Parco Tecnologico di Fuerteventura SA e responsabile del programma di innovazione Canarias Geo Innovation Program 2030. La sua traiettoria è stata riconosciuta con numerosi premi, come il Premio Distinto dell\'Anno di Fuerteventura "Onda Cero Radio", il Premio Spiagge d\'Oro del Consiglio di Fuerteventura nel 2005 e il Premio dell\'Associazione della Stampa di Fuerteventura e Lanzarote nel 1996 e 2004, oltre alla Medaglia d\'Argento del Turismo del Governo delle Canarie nel 1996 e il Premio Attualità Economica per la Migliore Iniziativa Pubblica (Smart Strategy) nel 2026. Nel 2025, è diventato membro del Comitato di Direzione di NEREUS, la rete delle regioni europee per l\'uso delle tecnologie spaziali.'
          },
          {
            name: 'Gonzalo Piernavieja Izquierdo',
            position: 'Direttore di R&S&I dell\'Istituto Tecnologico delle Canarie',
            expertise: 'Energie Rinnovabili',
            image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/GLOBAL%20STARTUP%2FMIEMBROS%20DE%20JURADO%2FGonzalo%20Piernavieja%20oct2021%203.jpg?alt=media&token=0482ecc5-f873-4dc5-9735-9040ea3ad2e7',
            bio: 'Gonzalo Piernavieja Izquierdo è laureato in Scienze Fisiche dell\'Università Ludwig-Maximilians di Monaco (1993) (specialità in energia solare e chimica-fisica) e ha realizzato un corso di laurea specialistica in Gestione Energetica e Ambientale dell\'Università Tecnica di Berlino (1994).',
            fullBio: 'Gonzalo Piernavieja Izquierdo è laureato in Scienze Fisiche dell\'Università Ludwig-Maximilians di Monaco (1993) (specialità in energia solare e chimica-fisica) e ha realizzato un corso di laurea specialistica in Gestione Energetica e Ambientale dell\'Università Tecnica di Berlino (1994). La sua tesi a Monaco si è centrata su un\'installazione fotovoltaica sperimentale da 1 kWp con inseguimento solare, uno dei primi sistemi connessi alla rete tedesca, più di 30 anni fa. Durante il suo soggiorno in Germania, Gonzalo Piernavieja ha lavorato in SIEMENS e, successivamente, nella compagnia di fornitura energetica di Monaco "Stadtwerke München". Tornato alle Canarie, ha coordinato, nell\'Università di Las Palmas de Gran Canaria, diversi progetti relativi alle energie rinnovabili e alle tecnologie dell\'acqua, nel quadro di programmi europei (1993-1996). Dal 1996 lavora nell\'Istituto Tecnologico delle Canarie (ITC) occupando diversi incarichi. Durante il 2018 e 2019 (fino ad agosto) è stato Vice-ministro dell\'Industria, Energia e Commercio del Governo delle Canarie. Attualmente è Direttore di R&S&I dell\'ITC, coordinando 9 dipartimenti tecnologici con più di 180 persone, con attività in energie rinnovabili, tecnologie dell\'acqua, metrologia, economia circolare, biotecnologia delle alghe, analisi ambientale, ingegneria meccanica e biomedica, informatica scientifica, innovazione e sistemi di informazione. Il suo principale ambito di specializzazione è l\'autosufficienza energetica e idrica, mediante energie rinnovabili, in isole e ambienti isolati in generale.'
          },
          {
            name: 'María Teresa Pérez Zaballos',
            position: 'CEO di Endogene.bio',
            expertise: 'Biotecnologia',
            image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/GLOBAL%20STARTUP%2FMIEMBROS%20DE%20JURADO%2FMar%C3%ADa%20Teresa%20P%C3%A9rez%20Zaballos%2C%20CEO%20de%20Endogene.bio.jpg?alt=media&token=d7a276a8-ef0c-43e7-bbdd-3aa714483e8a',
            bio: 'María Teresa Pérez Zaballos ha ottenuto un Master in Impresa Biosanitaria, Business e Biotecnologia nell\'Università di Cambridge, e ha completato una Certificazione Professionale in Genetica e Genomica nell\'Università di Stanford.',
            fullBio: 'María Teresa Pérez Zaballos ha ottenuto un Master in Impresa Biosanitaria, Business e Biotecnologia nell\'Università di Cambridge, e ha completato una Certificazione Professionale in Genetica e Genomica nell\'Università di Stanford. La sua esperienza si centra sulla salute femminile, così come sui dispositivi medici e bioelettronica. La sua specialità risiede nel design e nell\'esecuzione di attivi altamente regolati. Attualmente, è CEO dell\'impresa Endogene.bio, dove lavora quotidianamente per migliorare la qualità di vita delle donne con endometriosi.'
          },
          {
            name: 'Pedro Jesús Cabrera Santana',
            position: 'Professore Ordinario del Dipartimento di Ingegneria Meccanica dell\'ULPGC',
            expertise: 'Energie Rinnovabili',
            image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/GLOBAL%20STARTUP%2FMIEMBROS%20DE%20JURADO%2FPedro%20Jes%C3%BAs%20Cabrera%20Santana%20%E2%80%93%20Profesor%20Titular%E2%80%AFdel%20Departamento%20de%20Ingenier%C3%ADa%20Mec%C3%A1nica%20de%20la%20ULPGC%E2%80%AFde%20la%20Universidad%20de%20Las%20Palmas%20de%20Gran%20Canaria%20(ULPGC).png?alt=media&token=92828b28-b51b-4197-8a22-e5fa93b5682a',
            bio: 'Professore Ordinario del Dipartimento di Ingegneria Meccanica dell\'ULPGC, dove coordina il Group for the Research on Renewable Energy Systems (GRRES). La sua attività di ricerca si concentra sul nesso acqua-energia, con particolare enfasi sullo sviluppo di sistemi intelligenti.',
            fullBio: 'Professore Ordinario del Dipartimento di Ingegneria Meccanica dell\'ULPGC dell\'Università di Las Palmas de Gran Canaria (ULPGC), dove coordina il Group for the Research on Renewable Energy Systems (GRRES). La sua attività di ricerca si concentra sul nesso acqua-energia, con particolare enfasi sullo sviluppo di sistemi intelligenti che integrino energie rinnovabili nei processi di dissalazione e nella gestione efficiente delle risorse. Dottore Ingegnere Industriale e Ingegnere in Automatica ed Elettronica Industriale, ha iniziato la sua carriera nel 2008 in Red Eléctrica de España, contribuendo al piano di integrazione dell\'energia eolica nei sistemi elettrici delle Canarie. Da allora, ha spinto progetti pionieri nell\'azionamento di impianti dissalatori con energie rinnovabili, applicando tecniche di machine learning e controllo intelligente. La sua Tesi Dottorale ha ricevuto il Premio Menzione d\'Onore alla Migliore Tesi in Controllo Intelligente dal Comitato Spagnolo di Automatica. Ha ricoperto cariche di responsabilità come Coordinatore Nazionale del Gruppo Tematico di Controllo Intelligente del Comitato Spagnolo di Automatica (2020-2024), Editore della Sezione di Controllo Intelligente della Rivista Ibero-americana di Automatica e Informatica Industriale (RIAI) e Segretario Tecnico della Piattaforma R&S&I DESAL+ LIVING LAB, riferimento internazionale nell\'innovazione nella dissalazione. Autore e co-autore di multiple pubblicazioni scientifiche e libri, il suo lavoro combina ricerca applicata, trasferimento tecnologico e sostegno all\'imprenditorialità nel settore delle energie pulite. Attualmente guida lo sviluppo di strumenti e metodologie che contribuiscono alla pianificazione di sistemi energetici sostenibili a scala nazionale e internazionale.'
          },
          {
            name: 'Luis Suárez León',
            position: 'Vice-Presidente del Consorzio della Zona Speciale Canaria (ZEC)',
            expertise: 'Sviluppo Economico',
            image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/GLOBAL%20STARTUP%2FMIEMBROS%20DE%20JURADO%2FLuis%20Su%C3%A1rez%20Le%C3%B3n%20-%20Vicepresidente%20del%20Consorcio%20de%20la%20Zona%20Especial%20Canaria%20(ZEC)%20.png?alt=media&token=351c6802-2594-43a5-8890-61809b7e2769',
            bio: 'Dirigente con una traiettoria notevole nell\'amministrazione pubblica e nella gestione aziendale, attualmente Vice-Presidente del Consorzio della Zona Speciale Canaria (ZEC), entità chiave nello sviluppo economico e nell\'attrazione di investimenti nell\'arcipelago.',
            fullBio: 'Luis Suárez León è un dirigente con una traiettoria notevole nell\'amministrazione pubblica e nella gestione aziendale, che ha ricoperto rilevanti responsabilità istituzionali a livello regionale e ha consolidato una solida esperienza nella leadership di organizzazioni. Attualmente, svolge il ruolo di Vice-Presidente del Consorzio della Zona Speciale Canaria (ZEC), entità chiave nello sviluppo economico e nell\'attrazione di investimenti nell\'arcipelago. È stato dirigente di imprese legate agli impianti elettrici, manutenzioni industriali e costruzione; combinando visione strategica, conoscenza dell\'ambiente imprenditoriale canario e un fermo orientamento verso lo sviluppo sostenibile e innovativo. Durante la sua carriera, ha ricoperto vari incarichi di alta responsabilità nel Governo delle Canarie, tra cui Direttore Generale del Gabinetto della Vice-Presidenza, Direttore dell\'Area Sanitaria di Gran Canaria e Capo di Gabinetto nei Dipartimenti di Sanità, Economia e Industria, gestendo team multidisciplinari e promuovendo iniziative in politiche pubbliche, sanità ed economia. Luis Suárez León è Laureato in Scienze Politiche e dell\'Amministrazione presso l\'Università Complutense di Madrid, e ha formazione post-laurea in sistemi sanitari, competenze ICT e finanziamento di servizi pubblici. Inoltre, ha ampliato la sua formazione internazionale presso l\'Università di Essex (Regno Unito) e presso l\'Institut für Internationale Kommunikation (Germania).'
          },
          {
            name: 'Ana Torrent Acosta',
            position: 'Manager presso Canarias Excelencia Tecnológica',
            expertise: 'Tecnologia e Innovazione',
            image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/GLOBAL%20STARTUP%2FMIEMBROS%20DE%20JURADO%2FAna%20Torrent%20Acosta%20%E2%80%93%20%20Gerente%20en%20Canarias%20Excelencia%20Tecnol%C3%B3gica.png?alt=media&token=d6b12a77-86f7-449b-b07b-8dd581882b17',
            bio: 'Ingegnere delle Telecomunicazioni dell\'Università di Las Palmas de Gran Canaria e attualmente Manager di Canarias Excelencia Tecnológica, il cluster che riunisce le principali aziende del settore ICT nell\'arcipelago.',
            fullBio: 'Ana Torrent Acosta è ingegnere delle Telecomunicazioni dell\'Università di Las Palmas de Gran Canaria e attualmente esercita come Manager di Canarias Excelencia Tecnológica, il cluster che riunisce le principali aziende del settore ICT nell\'arcipelago. Da questa posizione impulsa progetti di innovazione, digitalizzazione e collaborazione pubblico-privata, convinta che la tecnologia costituisca una via strategica per diversificare l\'economia canaria. Inoltre, è vice-presidente di WISE Canarias, un\'organizzazione che incoraggia la partecipazione delle donne negli ambiti scientifici e tecnologici, e collabora in iniziative che promuovono le vocazioni STEAM tra i giovani. Durante la sua traiettoria ha lavorato nella gestione di programmi e aiuti orientati a migliorare la competitività e sostenibilità delle aziende, e partecipa attivamente a forum e incontri che diffondono il ruolo dell\'innovazione nello sviluppo sociale ed economico. Il suo lavoro combina la promozione del talento, l\'uguaglianza di genere nella tecnologia e la scommessa su un modello di sviluppo più digitale e inclusivo per le Canarie.'
          },
          {
            name: 'María Delgado Segura',
            position: 'Presidente di AJE Las Palmas',
            expertise: 'Imprenditorialità',
            image: 'https://firebasestorage.googleapis.com/v0/b/africagua-eb795.firebasestorage.app/o/GLOBAL%20STARTUP%2FMIEMBROS%20DE%20JURADO%2FMar%C3%ADa%20Delgado%20Segura%20es%20Presidenta%20de%20AJE%20Las%20Palmas.png?alt=media&token=a1b16fbb-de9c-41b4-b088-3a7646a44dc2',
            bio: 'Presidente di AJE Las Palmas da febbraio 2023, dove lavora per rafforzare l\'imprenditorialità giovanile e la collaborazione istituzionale nelle Canarie. È Co-fondatrice di Volcán Studio e Amministratore Delegato presso Flatguest.',
            fullBio: 'María Delgado Segura è Presidente di AJE Las Palmas da febbraio 2023, dove lavora per rafforzare l\'imprenditorialità giovanile e la collaborazione istituzionale nelle Canarie. È Co-fondatrice di Volcán Studio (2015), uno studio creativo multidisciplinare specializzato in progetti tecnici, design d\'interni e immagine aziendale per enti pubblici e privati. Inoltre, è Amministratore Delegato presso Flatguest dal 2019 e è stata Segretario Generale di AJE Canarias tra il 2021 e il 2023. Ha collaborato come relatrice per il Dipartimento dell\'Educazione delle Canarie, formando il corpo docente in strategie di marketing digitale applicate allo spazio commerciale. Ingegnere in Organizzazione Industriale e Ingegnere Tecnico in Design Industriale dell\'Università di Las Palmas de Gran Canaria, combina formazione tecnica con esperienza in gestione aziendale e innovazione. Con una solida traiettoria nel mondo del design e dell\'organizzazione, impulsa progetti che favoriscono la digitalizzazione, la creatività e la sostenibilità, difendendo sempre la voce e il ruolo della gioventù imprenditoriale nell\'arcipelago.'
          }
        ]
      }
    },
    map: {
      loading: 'Caricamento mappa...',
      error_title: 'Errore nel caricamento della mappa',
      loading_error: 'Impossibile caricare Google Maps. Controlla la tua connessione internet.',
      api_key_missing: 'Chiave API di Google Maps mancante. Configura VITE_GOOGLE_MAPS_API_KEY nel file .env',
      setup_instructions: 'Consulta il file .env.example per istruzioni su come configurare l\'API di Google Maps.',
      view_in_google_maps: 'Visualizza su Google Maps',
      show_route: 'Mostra percorso dall\'aeroporto',
      airport_route: 'Percorso Aeroporto',
      restaurants: 'Ristoranti',
      parking: 'Parcheggio',
      gas_stations: 'Stazioni di servizio',
      reset: 'Ripristina',
      fullscreen: 'Schermo intero',
      exit_fullscreen: 'Esci da schermo intero',
      interactive_map: 'Mappa Interattiva',
      simple_map: 'Mappa Semplice',
      nearby_restaurant: 'Ristoranti Vicini',
      nearby_parking: 'Parcheggi Vicini',
      nearby_gas_station: 'Stazioni di servizio Vicine'
    },
    thankYou: {
      title: 'Grazie!',
      message: 'L\'intero team di Africagua 2025 vi ringrazia per la vostra partecipazione a questa IX edizione del Forum Internazionale dell\'Acqua e delle Energie Rinnovabili.',
      future: {
        title: 'Ci vediamo nel 2027',
        message: 'Speriamo di rivedervi alla prossima edizione di Africagua Canarias 2027, dove continueremo a promuovere la cooperazione internazionale e lo sviluppo sostenibile tra Europa e Africa.'
      },
      signature: 'Il team di Africagua Canarias'
    }
  }
};